...
public final class UserManager extends SimpleDataHolder<UUID, UserImpl> implements Loadable {
    ...
    private DataStorage<UUID, UserImpl> storage;
    ...
    public void load() {
        ...
        boolean isMySql = setting.database().type().equalsIgnoreCase("mysql");
    ...
    public void enable() {
        this.autoSaveTask = AsyncScheduler.get(instance).runTimer(
                () -> save(),
                instance.get(SettingConfig.class).autoUpdateTime(),
                instance.get(SettingConfig.class).autoUpdateTime(),
                TimeUnit.SECONDS
        );
    }
    ...
    public void onCreate(DataEntry<UUID, UserImpl> entry) {
        Map<String, ItemImpl> map = instance.get(SettingConfig.class).getNormalizedWhitelist().stream()
        ...
        entry.setValue(user -> user.withItems(map).withSpace(instance.get(SettingConfig.class).maxSpace()), false);
    }
